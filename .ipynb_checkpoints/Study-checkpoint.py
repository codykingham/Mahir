    ...
    for term in deck:
        ...
        if TF:
            ...
            # get random example from TF
                ...
                ex_unit = TF.L.u(ex_instance, otype='verse')[0]
                
                # format highlighting for key term
                ex_text = ''
                for w in TF.L.d(ex_unit, 'word'):
                    w = TF.T.text(w) if w != ex_instance else f'•{TF.T.text(w)}'
                    ex_text += w

                ex_pass = '{} {}:{}'.format(*TF.T.sectionFromNode(ex_unit))
                example = f'{ex_pass}\t{ex_text}' 
                term_formatted = example # NB!! Redefines the term formatted text here! 
                definition = f'{term_text}\t{definition}'

        # present term to user for scoring
        displayNew('\n'+term_formatted)
 
         # COMMENT OUT FOR NOW, TRYING PASSAGES AS term_formatted
